...
# =========================
# GD5 APPEND-ONLY: Trap / Liquidity Event Tags
# =========================
def _gd5_block_from_signal(sig: Dict[str, Any]) -> str:
    try:
        if not isinstance(sig, dict):
            return ""
        meta = sig.get("meta") if isinstance(sig.get("meta"), dict) else {}
        session_v4 = meta.get("session_v4") if isinstance(meta.get("session_v4"), dict) else {}
        liquidation = meta.get("liquidation") if isinstance(meta.get("liquidation"), dict) else {}
        no_trade = meta.get("no_trade_zone") if isinstance(meta.get("no_trade_zone"), dict) else {}
        state = str(meta.get("market_state_v2") or "")
        ...
        liq_ok = bool(liquidation.get("ok"))
        liq_kind = str(liquidation.get("kind") or "")
        follow = str(session_v4.get("follow_through") or "")
        fake = str(session_v4.get("fake_move_risk") or "")
        reasons = [str(x) for x in (no_trade.get("reasons") or []) if x]

        if liq_ok:
            tags.append("ĐANG QUÉT SL")
            why.append(f"liquidation: {liquidation.get('side') or liq_kind}")
        if (liq_ok and follow == "NO") or fake == "HIGH" or state in ("CHOP", "POST_LIQUIDATION_BOUNCE", "POST_SHORT_COVER"):
            tags.append("BẪY 2 ĐẦU")
        if liq_ok or no_trade.get("active"):
            tags.append("KHÔNG TRADE ĐẦU SÓNG")
        if phase.get("phase") == 3 and phase.get("label") in ("EARLY", "EXTREME"):
            if "KHÔNG TRADE ĐẦU SÓNG" not in tags:
                tags.append("KHÔNG TRADE ĐẦU SÓNG")
        if reasons:
        ...
        for t in tags:
            if t not in uniq_tags:
                uniq_tags.append(t)
        lines = ["", "🧨 GD5 Trap Tags:", f"- Tag: {' | '.join(uniq_tags)}"]
        if why:
            lines.append(f"- Lý do: {'; '.join(why[:4])}")
        q = str(playbook_v4.get("quality") or "")
        if q:
            lines.append(f"- Playbook quality: {q}")
        return "\n".join(lines)
    except Exception:
        return ""

_format_signal_gd4_original = format_signal

def format_signal(sig: Dict[str, Any]) -> str:
    out = _format_signal_gd4_original(sig)
    extra = _gd5_block_from_signal(sig)
    if extra and extra not in out:
        out = str(out or "") + extra
    return out
