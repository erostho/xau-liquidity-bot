...
TERM_MAP = {
    ...
    "POST_LIQUIDATION_BOUNCE": "Sau nhịp quét mạnh, giá đang hồi lại",
...
def detect_liquidity(m15: List[Candle], atr: float) -> Dict[str, Any]:
    ...
    return {
        ...
        "liquidity_text": "Chưa thấy quét thanh khoản rõ ràng",
    }
...
def detect_confirmation(m15: List[Candle]) -> Dict[str, Any]:
    ...
    if break_up:
        ...
        text = "Đã có dấu hiệu giữ được đáy cao hơn"
    elif lh:
        text = "Đã có dấu hiệu giữ được đỉnh thấp hơn"
    else:
...
def detect_flow(symbol: str, bias_side: str) -> Dict[str, str]:
    ...
    return {"flow": "NEUTRAL", "flow_text": "Dòng tiền hiện chưa rõ ràng", "favored": bias_side if bias_side in ("BUY", "SELL") else "SIDEWAY"}
...
def detect_gap_context(m15: List[Candle], atr: float, bars_for_open: int = 4) -> GapContext:
    if len(m15) < bars_for_open + 2:
        return GapContext(
            ...
            note="Chưa đủ dữ liệu để đọc trạng thái mở cửa đầu phiên",
        )
    ...
    if unbalanced_open:
        ...
        note = "Có khoảng trống giá đầu phiên; chưa nên xem đây là tín hiệu vào lệnh ngay"
    elif has_wide_open_range:
        note = "Biên độ đầu phiên đang rộng bất thường; nên chờ ổn định hơn"
    else:
    ...
    try:
        ...
        if len(m15) < 5:
            return {
                ...
                "bias_text": bias_text,
                "status": "no_data",
                "message": "Bot chưa có đủ dữ liệu M15 để phân tích",
            }
        ...
        main_scenario = "Chờ tín hiệu rõ hơn"
        if bias_side == "SELL":
            main_scenario = "Ưu tiên chờ giá hồi yếu hơn rồi canh bán"
        elif bias_side == "BUY":
            main_scenario = "Ưu tiên chờ giá điều chỉnh nhẹ hơn rồi canh mua"

        caution: List[str] = []
        if gap.unbalanced_open:
            caution.append("Mở cửa đang mất cân bằng, dễ quét hai đầu")
        if liquidity.get("is_liquidation"):
            caution.append("Vừa có nhịp quét mạnh, không nên đuổi theo giá")
        if no_trade_zone:
        ...
        return {
            ...
            "bias_text": bias_text,
            "status": "error",
            "message": f"Phân tích bị lỗi ({symbol}): {e}",
        }
...
def review_manual_order(
    ...
    verdict = "CHƯA RÕ"
    note = "Cần thêm tín hiệu xác nhận"

    if side == analysis.get("bias_side") and rr >= 1.8:
        verdict = "TẠM ỔN"
        note = "Ý tưởng đang cùng hướng chính, nhưng vẫn cần quản trị chặt"

    if gap_context.get("unbalanced_open"):
        verdict = "THẬN TRỌNG"
        note = "Đầu phiên mở lệch mạnh; tránh thêm lệnh khi thị trường còn nhiễu"

    if liquidity.get("is_liquidation"):
        verdict = "THẬN TRỌNG"
        note = "Vừa có nhịp quét mạnh; không nên đuổi theo cây nến vừa chạy"

    if side == "SELL" and not confirmation.get("lh") and not confirmation.get("break_down"):
        note = "Ý tưởng bán có thể đúng hướng, nhưng vào lệnh còn sớm vì chưa có xác nhận giảm rõ"
    if side == "BUY" and not confirmation.get("hl") and not confirmation.get("break_up"):
        note = "Ý tưởng mua có thể đúng hướng, nhưng vào lệnh còn sớm vì chưa có xác nhận tăng rõ"

    manage = "Có thể giữ ngắn hạn, chưa nên add thêm"
    if analysis.get("no_trade_zone"):
        manage = "Ưu tiên giảm rủi ro, không add thêm, chờ thị trường ổn định hơn"
    ...
    return {
        ...
        "flow_text": analysis.get("flow", {}).get("flow_text", "Dòng tiền hiện chưa rõ"),
...
def render_analysis_text(result: Dict[str, Any]) -> str:
    if result.get("status") != "ok":
        return result.get("message", "Bot chưa đọc được đủ dữ liệu")
    ...
    lines = [
        f"📌 {result['symbol']} | NOW",
        f"🧭 Xu hướng chính hiện tại: {result['bias_text']}",
        f"💵 Giá hiện tại: {result['current_price']}",
        f"🪜 Giai đoạn 3-6-9 hiện tại: {result['phase_369']} | {result['phase_text']}",
        f"🌡 Trạng thái thị trường lúc này: {result['state_text']}",
        f"📍 Vị trí giá lúc này: {location['range_pos']}% trong biên độ M15 ({location['range_low']} – {location['range_high']})",
        f"💧 Thanh khoản: {liquidity['liquidity_text']}",
        f"✅ Tín hiệu xác nhận hiện tại: {confirmation['confirm_text']}",
        f"💰 Dòng tiền hiện tại: {flow['flow_text']}",
        f"🕳 GAP đầu phiên: {gap['note']}",
        f"🎯 Kịch bản chính lúc này: {result['main_scenario']}",
    ]
    ...
    if result.get("no_trade_zone"):
        lines.append("⛔ Đây là vùng nên đứng ngoài, hoặc chỉ quan sát thêm")

    if caution:
        lines.append("⚠️ Lưu ý thêm: " + " | ".join(caution))
...
def render_review_text(review: Dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"🧭 Xu hướng chính hiện tại: {review.get('bias_text', 'chưa rõ')}",
        f"🪜 Giai đoạn 3-6-9 hiện tại: {review['phase_369']} | {review['phase_text']}",
        f"🌡 Trạng thái thị trường lúc này: {review['state_text']}",
        f"📍 Vị trí giá lúc này: ~{review['range_pos']}% biên độ",
        f"💰 Dòng tiền hiện tại: {review['flow_text']}",
        f"💧 Thanh khoản: {review['liquidity_text']}",
        f"✅ Tín hiệu xác nhận hiện tại: {review['confirm_text']}",
        f"🕳 GAP đầu phiên: {review['gap_note']}",
        f"📌 Kết luận nhanh: {review['verdict']} — {review['note']}",
        f"⚙️ Hành động nên làm lúc này: {review['management']}",
    ]
    if plan:
        lines.extend([
            "📐 Gợi ý theo ATR:",
            ...
            f"- Khi lời khoảng +0.8 ATR → dời về hòa vốn quanh: {plan.get('be_at', 0.0)}",
            f"- Khi lời khoảng +1.2 ATR → bắt đầu trailing quanh: {plan.get('trail_at', 0.0)}",
        ])
    return "\n".join(lines)
