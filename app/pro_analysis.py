...
def format_signal(sig: Dict[str, Any]) -> str:
    ...
    session = sig.get("session") or meta.get("session") or ""
    ...
    # Score detail (4 pillars)
    if trade_mode in ("FULL", "HALF") and sd:
        lines.append("")
        lines.append(
            "Score detail: "
            f"Bias:{sd.get('bias_ok')} "
            f"Loc:{sd.get('location_ok', sd.get('pullback_ok'))} "
            f"Liq:{sd.get('liquidity_ok', sd.get('confluence_ok'))} "
            f"Conf:{sd.get('confirmation_ok', sd.get('momentum_ok'))}"
        )
