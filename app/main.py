...
# =========================
# GD5 APPEND-ONLY: append trap tags to REVIEW only
# =========================
def _gd5_review_block(symbol: str) -> str:
    try:
        data = _fetch_triplet(symbol, limit=260)
        sig = analyze_pro(symbol, data["m15"], data["m30"], data["h1"], data["h4"])
        if not isinstance(sig, dict):
            return ""
        ...
        tags = []
        why = []
        if liquidation.get("ok"):
            tags.append("ĐANG QUÉT SL")
            why.append(f"liquidation: {liquidation.get('side') or liquidation.get('kind')}")
        if (liquidation.get("ok") and str(session_v4.get("follow_through") or "") == "NO") or str(session_v4.get("fake_move_risk") or "") == "HIGH" or state in ("CHOP", "POST_LIQUIDATION_BOUNCE", "POST_SHORT_COVER"):
            tags.append("BẪY 2 ĐẦU")
        if liquidation.get("ok") or no_trade.get("active") or (phase.get("phase") == 3 and phase.get("label") in ("EARLY", "EXTREME")):
            tags.append("KHÔNG TRADE ĐẦU SÓNG")
        why.extend([str(x) for x in (no_trade.get("reasons") or []) if x][:3])
        tags2 = []
        for t in tags:
        ...
        if not tags2:
            return ""
        lines = ["", "🧨 GD5 Trap Tags:", f"- Tag: {' | '.join(tags2)}"]
        if why:
...
_review_manual_trade_original = review_manual_trade

def review_manual_trade(symbol: str, side: str, entry_lo: float, entry_hi: float, tp: float | None, sl: float | None) -> str:
    out = _review_manual_trade_original(symbol, side, entry_lo, entry_hi, tp, sl)
    extra = _gd5_review_block(symbol)
